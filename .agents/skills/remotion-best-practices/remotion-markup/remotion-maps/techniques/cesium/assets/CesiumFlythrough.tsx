...
import terrainPath from "./cesium-path.json" with { type: "json" };
import { type LngLat, smoothFlightPath } from "./flight-path.ts";

export type FlyoverMode = "landscape" | "city";
export type { LngLat } from "./flight-path.ts";
...
const loadCesium = () =>
  new Promise<any>((resolve, reject) => {
    if ((window as any).Cesium) {
      return resolve((window as any).Cesium);
    }
...
export const CesiumFlythrough: React.FC<CesiumFlythroughProps> = ({
  ...
  const [handle] = useState(() =>
    delayRender(`cesium init: ${mode}`, { timeoutInMilliseconds: 120_000 })
  );
